...
DATA_DIRECTORY = os.path.abspath(
    os.path.join(backend_directory, "scrapers", "merged_data.json")
)
STUDYPROGRAMS_DIRECTORY = os.path.abspath(
        ...
        with io.open(STUDYPROGRAMS_DIRECTORY, "r") as studyprograms_data:
            ...
            for studyprogram in studyprograms_json:
                if studyprogram["id"] not in studyprograms_dict.keys():
                    studyprograms_dict[studyprogram["id"]] = StudyProgram(
                        studyprogram["id"], studyprogram["name"], studyprogram["url"]
                    )
                ...
                for professor in professors:
                    if professor["id"] not in professors_dict.keys():
                        professors_dict[professor["id"]] = Professor(
                            professor["id"], professor["name"], professor["url"]
                        )
            ...
            for lecture in data_json:
                ...
                for timetable_entry in timetable_entries:
                    ...
                    temp_entry = Timetable(
                        ...
                        timetable_entry["time"]["to"],
                        timetable_entry["rhythm"],
                        duration,
                        duration_from,
                        duration_to,
                        timetable_entry["room"],
                        timetable_entry["status"],
                        timetable_entry["comment"],
                        timetable_entry["elearn"],
                        timetable_entry["einzeltermine_link"],
                        lecture["id"],
                        dates,
                    )
