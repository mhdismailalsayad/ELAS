...
class DataHandler {
  import_courses() {
    Backend.get("/e3selector/e3_courses_and_rating")
  ...
  constructor() {
    ...
    this.filterState =
      JSON.parse(localStorage.getItem("e3filters")) ||
      require("./data/filters.json");

    const preSelectedCourses =
      JSON.parse(localStorage.getItem("e3selected")) || [];
  ...
  setStudyProgram(program) {
    this.filterState.Ausgeschlossen_Ingenieurwissenschaften_Bachelor[
      program
    ] = false;

    if (program === "Bauingenieurwesen") {
      this.filterState.Ausgeschlossen_Ingenieurwissenschaften_Bachelor[
        "ALLE (außer Bauingenieurwesen (1. FS))"
      ] = true;
      this.filterState.Ausgeschlossen_Ingenieurwissenschaften_Bachelor[
        "ALLE (außer Bauingenieurwesen)"
      ] = true;
    }
  }
  ...
  setFilter(family, item) {
    if (family === "credits") {
      ...
      if (item === "all") {
        Object.keys(this.filterState.catalog).forEach(
          (k) => (this.filterState.catalog[k] = true)
        );
      } else {
        Object.keys(this.filterState.catalog).forEach(
          (k) => (this.filterState.catalog[k] = false)
        );
  ...
  setSorting(key) {
    var direction =
      key === this.sortState.key ? this.sortState.direction * -1 : 1;
  ...
  handleSelection(course) {
    ...
    const credits = course.Credits.includes("-")
      ? course.Credits.split("-")
      : Array(2).fill(course.Credits);

    if (e !== undefined) {
      // e!== undefined means e is currently a selected course
      this.selectedList = this.selectedList.filter((c) => c.Title !== e.Title);

      this.selectedSWS -= sws;
      this.selectedCredits = this.selectedCredits.map(
        (n, i) => n - parseInt(credits[i])
      ); // credits are stored in a weird format in the json. not our decision.
      slots.forEach((time, t) => {
        this.bookedTimeSlots[time] -= 1;
      });
    } else {
      this.selectedList = this.selectedList.concat(
        this.courseList.find((c) => c.Title === course.Title)
      );
      ...
      slots.forEach((time, t) => {
        this.bookedTimeSlots[time] = this.bookedTimeSlots[time] + 1 || 1;
      });
    }
  ...
  getUnselectedCourses() {
    return this.courseList.filter(
      (c) => !this.selectedList.map((s) => s.Title).includes(c.Title)
    );
  }
  ...
  getOverBookedTimeSlots() {
    return Object.keys(this.bookedTimeSlots)
      .map((k, i) => (this.bookedTimeSlots[k] > 1 ? k : null))
      .filter((b) => b);
  }
  ...
  getCredits() {
    ...
    return this.selectedCredits[0] === this.selectedCredits[1]
      ? this.selectedCredits[0]
      : this.selectedCredits[0] + "-" + this.selectedCredits[1];
  }

  /* return values are used as classes for styling the overview notifications */
  getCreditsStatus() {
    if (
      this.selectedCredits[0] === this.filterState.credits ||
      this.selectedCredits[1] === this.filterState.credits
    ) {
      return "on-ok";
    } else if (
      this.selectedCredits[0] > this.filterState.credits &&
  ...
  getSmallCourses() {
    ...
    this.selectedList.forEach((item, i) => {
      if (
        parseInt(item["Erwartete Teilnehmer"]) <= this.smallCourseThreshold ||
        parseInt(item["Max. Teilnehmer"].split(";")[0]) <=
          this.smallCourseThreshold
      ) {
        small = true;
      }
    });
  ...
  applyFilters(data) {
    this.courseList = data.filter((course) => {
      ...
      // Study Program
      let exempt =
        course.Ausgeschlossen_Ingenieurwissenschaften_Bachelor.split(/,|;/);
      ...
      if (
        part > 0 &&
        (this.filterState.participants[0] > part ||
          this.filterState.participants[1] < part)
      ) {
        fitting = false;
        console.log(part);
      }
