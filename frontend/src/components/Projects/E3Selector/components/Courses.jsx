...
const borderSelect = (type) => {
  switch (type) {
    case "VL/Übung":
      return "lecExBorder";
    case "Vorlesung":
      return "lectureBorder";
    case "Blockseminar":
      return "blockBorder";
    case "Seminar":
      return "seminarBorder";
    case "E-Learning":
      return "elearnBorder";
    case "Projektseminar":
      return "seminarBorder";
  ...
  return (
    ...
    >
      ...
      </Grid>
      {
        ...
        list
          .filter((c) => !selectedList.map((s) => s.Title).includes(c.Title))
          .map((entry) => {
            return (
              <Grid item xs={12}>
                <Course
          ...
          return (
            <Grid item xs={12}>
              <Course
                ...
                selected={true}
                booked={props.booked}
                overBooked={props.overBooked}
                handleSel={props.handleSel}
              />
            </Grid>
          );
        })}
...
const Course = (props) => {
  ...
  return (
    <Paper
      ...
      className={classes[borderSelect(Type.split(";")[0])]}
    >
      <div class="select-icon">
        <IconButton onClick={() => handleSel(props)}>
      ...
      <Grid item xs={12}>
        ...
        >
          ...
          <Grid
            ...
            onClick={() => toggle(!isOpen)}
          >
            <img class="lang-flag" alt={Language} src={langFlag(Language)} />
          </Grid>
          <Grid item xs={1} className={classes.emphasis}>
            <div class="expand-icon">
              <Icon aria-label="expand row" onClick={() => toggle(!isOpen)}>
      ...
      <Collapse in={isOpen} timeout="auto" unmountOnExit>
        ...
        <Grid item xs={12} style={{ padding: 24 }}>
          <Grid
            ...
            justify="space-evenly"
          >
            <Grid item xs={12} md={selected ? 12 : 6} xl={selected ? 6 : 3}>
              <RChart
                fairness={fairness}
                support={support}
                material={material}
            ...
            <Grid item xs={12} md={selected ? 12 : 6} xl={selected ? 6 : 4}>
              <Schedule
                schedule={schedule}
                booked={booked}
            ...
            <Grid item xs={12} md={selected ? 12 : 6} xl={selected ? 6 : 2}>
              <div class="info-table">
                <table>
                  ...
                  <tr>
                    <th>Course Type:</th>
                    <td class={borderSelect(Type.split(";")[0])}>
                      {Type.split(";")
                        .map((e) => fType(e))
                        .join(", ")}
                    </td>
